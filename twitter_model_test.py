...
#this file is an edited version of the twitter model that made some improvements such as actually saving the model weights and was used to make writing the blog post easier
...
model.fit(training_padded, training_labels, epochs=num_epochs, validation_data=(testing_padded, testing_labels), verbose=2, callbacks=[weights_checkpoint] )
model.load_weights(checkpoint_path)

words = ["I really like dogs", "I hate doing homework", "I am ok", "I am not ok","kill kill kill kill kill kill", "I want to kill myself", "I want to kill kill kill myself", "Where is the bus?", "what time is it?", "How is it?", "Trump is a good president", "I am glad that he is not sick"]
to_predict = pad_sequences(tokenizer.texts_to_sequences(words),maxlen=max_length, padding=padding_type, truncating=trunc_type)

print(model.predict(to_predict, verbose=1))
